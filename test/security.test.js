...
describe('Security: source address validation (RFC 6762 §11)', () => {
  // RFC 6762 §11 requires that mDNS responses received via link-local
  // multicast are "necessarily deemed to have originated on the local link,
  // regardless of source IP address." Since this library only listens on
  // the multicast socket, all received packets are inherently local.
  //
  // As defense-in-depth, the transport rejects packets with clearly invalid
  // source addresses (0.0.0.0 and ::). Node.js dgram does not allow forging
  // source addresses, so we cannot send a packet with rinfo.address === '0.0.0.0'
  // in a real network test. Instead, we verify the check exists by reading
  // the transport code and testing that valid packets are still accepted.
  ...
  test('packets from valid source addresses are accepted (baseline)', async () => {
    // This test verifies that the source address check does not reject
    // legitimate packets. Valid multicast responses from real network
    // interfaces always have a non-zero source address.
    const browser = mdns.browse('_http._tcp')
    const iter = browser[Symbol.asyncIterator]()

    await advertiser.announce({
      name: 'SourceCheckService',
      type: '_http._tcp',
      host: 'source-check.local',
      port: 7777,
      addresses: ['10.0.0.42'],
    })
    ...
    const event = await nextEvent(iter, 5000)
    assert.equal(event.type, 'serviceUp')
    assert.equal(event.service.name, 'SourceCheckService')

    browser.destroy()
  })

  test('transport code contains defense-in-depth source address check', async () => {
    // Verify the source address validation exists in the transport module.
    // We read the source to confirm the check is present — this guards
    // against accidental removal during refactoring.
    const { readFile } = await import('node:fs/promises')
    const { fileURLToPath } = await import('node:url')
    const { dirname, join } = await import('node:path')
    const __dirname = dirname(fileURLToPath(import.meta.url))
    const source = await readFile(join(__dirname, '..', 'lib', 'transport.js'), 'utf8')

    assert.ok(
      source.includes("rinfo.address === '0.0.0.0'"),
      'transport.js should reject packets from 0.0.0.0'
    )
    assert.ok(
      source.includes("rinfo.address === '::'"),
      'transport.js should reject packets from ::'
    )
  })
})
