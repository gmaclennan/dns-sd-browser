...
function decodeTxtData(buf, offset, rdlength) {
  ...
  while (offset < end) {
    ...
    // Preserve zero-length strings: RFC 6763 §6.1 mandates a single zero
    // byte (= one empty string) for TXT records with no service-defined
    // data, so the count of strings is meaningful and must round-trip.
...
function decodeName(buf, offset) {
  ...
  // Root-only name (single zero byte on the wire) renders as "." per
  // standard DNS convention; "" would be ambiguous with "no name".
  return { name: labels.length === 0 ? "." : labels.join("."), offset: returnOffset };
}
...
function encodeName(name, compressionTable, currentOffset) {
  // Root name encodes as a single zero byte (RFC 1035 §3.1). Accept
  // either "." (canonical) or "" (legacy/empty) to match decodeName's
  // historical output.
  if (name === "" || name === ".") {
