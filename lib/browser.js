...
export class ServiceBrowser {
   ...
   * Maps service FQDN → { queryCount: number, firstSeenAt: number }
   * Per RFC 6762 §10.5, after 2+ unanswered queries within the timeout window,
   * the record should be flushed from the cache.
   * @type {Map<string, { queryCount: number, firstSeenAt: number }>}
   */
  #poofTracking = new Map()

  /**
   * Active POOF response-wait timers, keyed by a unique query observation ID.
   * Each timer waits a short window for a response; if none arrives, the POOF
  ...
  /**
   * Timeout (ms) for POOF window per RFC 6762 §10.5 (default 10 seconds).
   * After 2+ unanswered queries within this window, cached records are flushed.
   */
  #poofTimeoutMs

  /**
   * Delay (ms) to wait for a response after observing a query before counting
   * it as unanswered.
   */
  #poofResponseWaitMs
   ...
   */
  constructor(transport, { queryName, serviceType, domain, protocol, isTypeEnumeration, signal, onDestroy, poofTimeoutMs, poofResponseWaitMs }) {
  ...
  #handleQuery = (packet) => {
    ...
    // Only interested in queries for our service type
    const questions = packet.questions || []
    const isRelevant = questions.some(
      (q) => q.name.toLowerCase() === this.#queryName.toLowerCase() && q.type === RecordType.PTR
    )
    if (!isRelevant) return

    // For each cached service FQDN, check if this query would expect our
    // cached PTR record as an answer. Start a response-wait timer.
    const now = Date.now()
    for (const fqdn of this.#knownPtrRecords.keys()) {
      // Skip if there's already a pending response-wait timer for this FQDN
      if (this.#poofTimers.has(fqdn)) continue

      const timer = setTimeout(() => {
        this.#poofTimers.delete(fqdn)
        // No response was seen — increment the POOF counter
        const tracking = this.#poofTracking.get(fqdn)
        if (tracking) {
          tracking.queryCount++
          // If the first query was seen more than poofTimeoutMs ago, reset
          if (now - tracking.firstSeenAt > this.#poofTimeoutMs) {
            tracking.queryCount = 1
            tracking.firstSeenAt = now
          }
        } else {
          this.#poofTracking.set(fqdn, { queryCount: 1, firstSeenAt: now })
        }

        // After 2+ unanswered queries, flush the record
        const current = this.#poofTracking.get(fqdn)
        if (current && current.queryCount >= 2 && (now - current.firstSeenAt) <= this.#poofTimeoutMs) {
          this.#poofTracking.delete(fqdn)
          this.#removeService(fqdn)
        }
   ...
   * Called from #handlePacket when we see a PTR record confirming the service.
