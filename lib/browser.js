...
export class ServiceBrowser {
  ...
  destroy(reason) {
    ...
    // If destroyed with a reason, reject so the iterator throws
    // (matching Node.js convention). Otherwise resolve for a clean exit.
    if (this.#eventSignal) {
      if (this.#destroyReason !== undefined) {
        this.#eventSignal.reject(this.#destroyReason)
      } else {
      ...
      next: async () => {
        while (true) {
          ...
          if (this.#destroyed) {
            ...
            /* c8 ignore next 2 -- destroy reason is thrown via deferred rejection in practice */
            if (this.#destroyReason !== undefined) {
...
export class AllServiceBrowser {
  ...
  destroy(reason) {
    ...
    // If destroyed with a reason, reject so the iterator throws
    // (matching Node.js convention). Otherwise resolve for a clean exit.
    /* c8 ignore start -- reason rejection mirrors ServiceBrowser */
    if (this.#eventSignal) {
      if (this.#destroyReason !== undefined) {
        this.#eventSignal.reject(this.#destroyReason)
      } else {
