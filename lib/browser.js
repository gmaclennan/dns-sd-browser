...
/**
 * How often to check for expired TTLs (ms).
 * RFC 6762 §5.2 recommends re-querying at 80%, 85%, 90%, 95% of TTL.
 * We check every 10s which is sufficient for typical TTLs (120s–4500s).
 */
...
export class ServiceBrowser {
  ...
  constructor(transport, { queryName, serviceType, domain, protocol, isTypeEnumeration, signal }) {
    ...
    this.#ttlCheckTimer = setInterval(() => this.#expireStaleRecords(), TTL_CHECK_INTERVAL_MS)
   ...
   * Called periodically by #ttlCheckTimer (RFC 6762 §5.2).
   */
  #expireStaleRecords() {
    if (this.#destroyed) return
