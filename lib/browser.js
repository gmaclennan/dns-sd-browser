  ...
  #handleQuery = (packet) => {
    ...
    for (const fqdn of this.#knownPtrRecords.keys()) {
      ...
      const timer = setTimeout(() => {
        ...
        if (firedAt - current.firstSeenAt > this.#poofTimeoutMs) {
          current.queryCount = current.pendingQueries
          current.firstSeenAt = firedAt
        } else {
          current.queryCount += current.pendingQueries
        }
        ...
        if (current.queryCount >= 2 && (firedAt - current.firstSeenAt) <= this.#poofTimeoutMs) {
          this.#poofTracking.delete(fqdn)
          // Also clean up PTR cache and pending state for unresolved instances
          this.#knownPtrRecords.delete(fqdn)
          this.#pendingFqdns.delete(fqdn)
          this.#removeService(fqdn)
        }
