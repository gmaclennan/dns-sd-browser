...
export class ServiceBrowser {
  ...
  #handleIncomingQuery = (packet) => {
    ...
    // Check if any question matches our query
    const hasMatchingQuestion = packet.questions.some(
      (q) => q.type === RecordType.PTR && q.name.toLowerCase() === this.#queryName.toLowerCase()
    )
    if (!hasMatchingQuestion) return

    // Check if their known answers cover ours
    // (their known-answer set must include all records we know about)
    const theirAnswers = new Set(
      (packet.answers || [])
        .filter((a) => a.type === RecordType.PTR)
        .map((a) => /** @type {string} */ (a.data)?.toLowerCase())
    )

    const allCovered = [...this.#knownPtrRecords.keys()].every(
      (fqdn) => theirAnswers.has(fqdn.toLowerCase())
    )
