...
export class ServiceBrowser {
   ...
   * Used for known-answer suppression.
  ...
  async #sendQuery() {
    ...
    try {
      await this.#transport.sendQuery({
        questions: [{ name: this.#queryName, type: RecordType.PTR }],
  ...
  #handlePacket = (packet) => {
    ...
    // Merge answers and additionals — both can contain relevant records
    ...
    for (const record of allRecords) {
      ...
      if (record.ttl === 0) {
        // Goodbye packet (RFC 6762 §10.1)
        this.#removeService(serviceFqdn)
        continue
      }

      // Track for known-answer suppression
  ...
  #resolveService(serviceFqdn, records) {
    ...
    const srvRecord = records.find(
      (r) => r.type === RecordType.SRV && r.name === serviceFqdn
    )

    // Find TXT record for this service
    const txtRecord = records.find(
      (r) => r.type === RecordType.TXT && r.name === serviceFqdn
    )
    ...
    if (!srvRecord && !existing) {
      ...
      // Can't resolve without SRV — wait for a future response
      return
    }
  ...
  #handleRecordUpdates(records) {
    for (const [fqdn, service] of this.services) {
      // Check for TXT updates
      const txtRecord = records.find(
        (r) => r.type === RecordType.TXT && r.name === fqdn
      )
      if (txtRecord) {
      ...
      const srvRecord = records.find(
        (r) => r.type === RecordType.SRV && r.name === fqdn && r.ttl === 0
      )
      if (srvRecord) {
  ...
  #collectAddresses(hostname, records) {
    ...
    for (const record of records) {
      if (record.name !== hostname) continue
