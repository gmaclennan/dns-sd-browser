...
export class DnsSdBrowser {
   ...
   * @param {number} [options.poofTimeoutMs] - POOF window in ms (RFC 6762 §10.5, default 10000)
   * @param {number} [options.poofResponseWaitMs] - Response wait time in ms (default 2000)
  ...
  browseAll(options = {}) {
    ...
    /** @type {{ ref: AllServiceBrowser | null }} */
    const holder = { ref: null }
    const browsers = this.#browsers
    const browser = new AllServiceBrowser(this.#transport, {
      domain: DEFAULT_DOMAIN,
      signal: options.signal,
      onDestroy: () => { if (holder.ref) browsers.delete(holder.ref) },
    })
    holder.ref = browser
