...
export class MdnsTransport {
  ...
  #onMessage(msg, rinfo) {
    try {
      // RFC 6762 §6: source UDP port in all Multicast DNS responses MUST be 5353.
      // Silently ignore responses from other ports.
      ...
      if (!packet.flags.qr) {
        // Query packet (QR=0) — dispatch to query handlers for POOF
   ...
   * Used for Passive Observation of Failures (RFC 6762 §10.5).
   * @param {PacketHandler} handler
   */
